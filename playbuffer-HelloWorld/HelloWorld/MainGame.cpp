...
{
	int score = 0;
	Agent8State agantState = STATE_APPEAR;
};
GameState gameState;
...
{
	Play::DrawBackground();
	UpdateAgent8(); //HandlePlayerControls();
	UpdateFan();
	UpdateTools();
	UpdateCoinsAndStars();
	UpdateLasers();
	UpdateDestroyed();
	if (gameState.agantState == STATE_DEAD)
	{
		Play::DrawFontText("64px", "ARROW KEYS TO MOVE UP AND DOWN AND SPACE TO FIRE", { DISPLAY_WIDTH / 2,50 }, Play::CENTRE);
	}
	else
	{
		Play::DrawFontText("132", "SCORE : " + std::to_string(gameState.score), { DISPLAY_WIDTH / 2,50 }, Play::CENTRE);
	}
...
void UpdateAgent8()
{
	...
	switch (gameState.agantState)
	{
		case STATE_APPEAR:
			...
			if (obj_agent8.pos.y >= DISPLAY_HEIGHT / 3)
				gameState.agantState = STATE_PLAY;
			break;
		...
		case STATE_DEAD:
			...
			{
				gameState.agantState = STATE_APPEAR;
	...
	if (Play::IsLeavingDisplayArea(obj_agent8) && gameState.agantState != STATE_DEAD)
...
void UpdateTools()
{
	...
	for (int id : vTools)
	{
		...
		if (gameState.agantState != STATE_DEAD && Play::IsColliding(obj_tool, obj_agent8))
		{
			Play::StopAudioLoop("music");
			Play::PlayAudio("die");
			obj_agent8.pos = { -100, -100 };
			gameState.agantState = STATE_DEAD;
		}
...
void HandlePlayerControls()
{
	...
	else
	{
		...
		{
			gameState.agantState = STATE_HALT;
			Play::SetSprite(obj_agent8, "agent8_halt", 0.33f);
	...
	{
		Vector2D firePos = obj_agent8.pos + Vector2D(115, -75);
